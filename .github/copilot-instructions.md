...
- **AccessDenied flow:** Use `Draw403` SVG from `Craft.UiBuilders.SVGs`; `AccessDeniedUser` component and `RedirectToAccessDenied` are in `Craft.AppComponents.Security.UI.Auth`. The GccPT `AccessDenied` page is located in `Pages/Auth` (not `Pages/Common`) using `BlankLayout` and `AllowAnonymous`. The `Routes.razor` splits `NotAuthorized` — authenticated-but-unauthorized redirects to `/access-denied`, while unauthenticated users are redirected to `/login`.
...
- In `YearHttpServiceTests`, ensure `_yearCounter` cycles within `[IYear.MinYearNumber, IYear.MaxYearNumber]` (2000–2100) using modulo arithmetic to prevent overflow. Use the formula: `IYear.MinYearNumber + (Interlocked.Increment(ref _yearCounter) % (IYear.MaxYearNumber - IYear.MinYearNumber + 1))`.
...
## 6 · AuthControllerBase Specific Rules

- In `LoginAsync` and `RefreshTokenAsync`, return `Unauthorized()` with a non-empty `List<string>` body.
- In `ChangePasswordAsync` and `ResetPasswordAsync`, wrap the repository call in a try/catch and return `BadRequest` with a sanitized message.
- In `ForgotPasswordAsync`, avoid swallowing exceptions silently and always return `NoContent`.
